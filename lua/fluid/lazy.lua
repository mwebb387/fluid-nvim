...
local M = {
  pending = {}, -- plugin name -> install path
...
function M.register(plug_data)
  local name = vim.fn.fnamemodify(plug_data.path, ':t')
  if not M.loaded[name] then
    M.pending[name] = plug_data.path
  end
end

function M.path_of(name)
...
function M.load(name)
  ...
  if vim.v.vim_did_enter == 1 then
    for _, p in ipairs(glob_scripts(path .. '/after/plugin/**/*')) do
